 ...
 * \class QAbstractAxis
 * \inmodule Qt Charts
 * \brief QAbstractAxis 类主要用于操作图表的坐标轴。
 *
 * 每一个序列可以关联到一个或者多个横轴和纵轴，但是混合可能会产生不同domain的坐标轴类型的做法是不支持的，例如在同一
 * 方向（orientation）上指定 QValueAxis 和 QLogValueAxis 。
 *
 * 不同坐标轴元素如坐标轴线，标题，标签，网格线，以及遮蔽（shades）的属性和可见性可以被分别控制。
 */
/*!
 ...
 *
 * \brief AbstractAxis 是特定坐标轴元素的通用基类元素。
 *
 * 每一个序列可以只能关联到一个横轴和一个纵轴。
 *
 * 不同坐标轴元素如坐标轴线，标题，标签，网格线，以及遮蔽（shades）的属性和可见性可以被分别控制。
 */
 ...
 * 次级（minor）网格线的可见性。仅应用于 QValueAxis。
 */
/*!
 ...
 * 次级（minor）网格线的画笔。仅应用于 QValueAxis 。
 */
 ...
 * 次级（minor）网格线的颜色。仅应用于 QValueAxis 。
 */
