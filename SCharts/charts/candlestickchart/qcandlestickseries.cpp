 ...
 *
 * \note 在一个 QCandlestickSeries 对象中，时间戳不能出现重复。当使用 QBarCategoryAxis 时，使用相同时间戳的柱线
 * 中的第一个才会被绘制出来。如果一个图表包含了多个 QCandlestickSeries 实例，则来自不同序列而具有相同时间戳的柱线
 * 将绘制在统一个类别（category）中。当使用 QValueAxis 或者 QDateTimeAxis时，使用同一个时间戳的柱线将会相互覆盖。
 *
