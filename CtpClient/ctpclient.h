...
class CTPCLIENTSHARED_EXPORT CtpClient : public CThostFtdcTraderSpi
{
public:
    static CtpClient& instance();

    bool isConnected() const;

    CtpClientNotifier& notifier();
    ...
    ///锁定应答
    // virtual void OnRspLockInsert(CThostFtdcInputLockField *pInputLock, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)  override;
    ...
    ///请求查询锁定应答
    // virtual void OnRspQryLock(CThostFtdcLockField *pLock, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)  override;

    ///请求查询锁定证券仓位应答
    // virtual void OnRspQryLockPosition(CThostFtdcLockPositionField *pLockPosition, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)  override;

    ///请求查询ETF期权合约手续费响应
    // virtual void OnRspQryETFOptionInstrCommRate(CThostFtdcETFOptionInstrCommRateField *pETFOptionInstrCommRate, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)  override;

    ///请求查询投资者分级
    // virtual void OnRspQryInvestorLevel(CThostFtdcInvestorLevelField *pInvestorLevel, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)  override;

    ///请求查询E+1日行权冻结响应
    // virtual void OnRspQryExecFreeze(CThostFtdcExecFreezeField *pExecFreeze, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)  override;
    ...
    ///锁定通知
    // virtual void OnRtnLock(CThostFtdcLockField *pLock)  override;

    ///锁定错误通知
    // virtual void OnErrRtnLockInsert(CThostFtdcInputLockField *pInputLock, CThostFtdcRspInfoField *pRspInfo)  override;
    ...
    CThostFtdcTraderApi* getTraderApi(const std::string &bacid);
    CThostFtdcTraderApi* CreateTraderApi(const std::string &bacid);

    static bool checkRequestResult(int result, std::string& errMsg);
...
private:
    ...
    CtpClientNotifier* mNotifier;

    int mRequestId = 0;
    std::map<std::string, CThostFtdcRspUserLoginField> userLogin;
    std::map<std::string, CThostFtdcTraderApi*> userTraderApi;
    std::mutex traderApiMutex;

    std::map<int, uint64_t> reqIdSession;
    std::map<int, int> reqIdRecvTime;
};
